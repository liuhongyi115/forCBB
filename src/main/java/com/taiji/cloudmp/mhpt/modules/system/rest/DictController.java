...
//    }
    ...
    @GetMapping(value = "/dict")
    @PreAuthorize("hasAnyRole('ADMIN','DICT_ALL','DICT_SELECT')")
    public ResponseEntity getDicts(DictQueryCriteria criteria) {
        List<DictDTO> dictDTOS = dictService.queryAll(criteria);
        return new ResponseEntity(dictService.buildTree(dictDTOS), HttpStatus.OK);
    }
